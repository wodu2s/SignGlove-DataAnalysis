...
def load_and_merge_episode_data(target_char, episode):
    ...
    if all_missing_reports:
        ...
        logging.error(f"결측치가 발견되어 처리를 중단합니다. '{report_filename}' 파일을 확인하세요.")
        return None, None
...
def process_target(target_char):
    ...
    for episode in EPISODE_DIRS:
        ...
        episode_df, numeric_cols = load_and_merge_episode_data(target_char, episode)
        ...
        episode_summary.to_csv(episode_filename, encoding='utf-8-sig')
        logging.info(f"에피소드 요약 파일 저장 완료: {episode_filename}")
    ...
    total_summary.to_csv(total_filename, encoding='utf-8-sig')
    logging.info(f"전체 통합 요약 파일 저장 완료: {total_filename}")
